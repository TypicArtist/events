...
public class EventBus {
    ...
    public <T> T post(T event) {
        ...
        for (Class<?> type : resolveHierarchy(event.getClass())) {
            ...
            for (EventListener l : list) {
                ...
                if (l.isOnce()) list.remove(l);

                if (cancellable != null && cancellable.isCancelled()) return event;
            }
        }
    ...
    public void register(Object owner) {
        for (Method m : owner.getClass().getDeclaredMethods()) {
            ...
            try {
                ...
                addListener(type, new MethodListener(owner, type, handle, meta.priority().value(), meta.once(), sequence.getAndIncrement()));
    ...
    public void unregister(Object owner) {
        for (List<EventListener> list : listeners.values()) list.removeIf(l -> l.getOwner() == owner);
    }
    ...
    public void setExceptionHandler(Consumer<Throwable> handler) { this.exceptionHandler = Objects.requireNonNull(handler); }
